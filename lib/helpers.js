...
export const languages = b5.getInfo()
  ...
  .filter(lang => lang.value === 'fr' || lang.value === 'en')
...
export const formatId = id => /^((http|https):\/\/)/.test(id) ? id.replace((process.env.NODE_ENV === 'development' ? 'http://localhost:8080' : 'https://hypatie.africitizen.co') + '/result/', '').replace(' ', '').toLowerCase() : id ? id.replace(' ', '') : id
